...
$students = [
    ['CSE-001', 'Aarav Sharma', 'aarav@college.local', '9000011111', 'Computer Science', 3, 2024, $studentUserId],
    ['CSE-002', 'Isha Mehta', 'isha@college.local', '9000012121', 'Computer Science', 3, 2024, null],
    ['ECE-014', 'Diya Patel', 'diya@college.local', '9000022222', 'Electronics', 5, 2023, null],
    ['ME-022', 'Rahul Verma', 'rahul@college.local', '9000033333', 'Mechanical', 2, 2025, null],
];

$stmtStudent = $pdo->prepare(
    'INSERT IGNORE INTO students (user_id, roll_number, full_name, email, phone, department, semester, batch_year)
     VALUES (:user_id, :roll_number, :full_name, :email, :phone, :department, :semester, :batch_year)'
);

foreach ($students as [$roll, $name, $email, $phone, $department, $semester, $batchYear, $userId]) {
    $stmtStudent->execute([
        'user_id' => $userId,
        'roll_number' => $roll,
        'full_name' => $name,
